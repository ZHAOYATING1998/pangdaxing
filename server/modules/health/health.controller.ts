...
export class HealthController {
  ...
  async getStats(@Req() req: any) {
    return this.healthService.getStats(req.user.user_id);
  }

  // ==================== 体重 ====================
  @Get('weight')
  async getWeightRecords(@Req() req: any, @Query('days') days?: number) {
    return this.healthService.getWeightRecords(req.user.user_id, days || 30);
  }
  ...
  async deleteWeightRecord(@Req() req: any, @Param('id') id: string) {
    return this.healthService.deleteWeightRecord(id, req.user.user_id);
  }

  // ==================== 饮食 ====================
  @Get('diet')
  async getDietRecords(@Req() req: any, @Query('date') date?: string) {
    return this.healthService.getDietRecords(req.user.user_id, date);
  }
  ...
  async deleteDietRecord(@Req() req: any, @Param('id') id: string) {
    return this.healthService.deleteDietRecord(id, req.user.user_id);
  }

  // ==================== 运动 ====================
  @Get('exercise')
  async getExerciseRecords(@Req() req: any, @Query('days') days?: number) {
    return this.healthService.getExerciseRecords(req.user.user_id, days || 30);
  }
  ...
  async deleteExerciseRecord(@Req() req: any, @Param('id') id: string) {
    return this.healthService.deleteExerciseRecord(id, req.user.user_id);
  }

  // ==================== 目标 ====================
  @Get('goals')
  async getGoals(@Req() req: any) {
    return this.healthService.getGoals(req.user.user_id);
  }
  ...
  async updateGoal(@Req() req: any, @Param('id') id: string, @Body() body: any) {
    return this.healthService.updateGoal(id, req.user.user_id, body);
  }
}
